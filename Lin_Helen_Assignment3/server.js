...
function query_DB(POST, response) {
  if (
    isNonNegativeInteger(POST["low_price"]) &&
    isNonNegativeInteger(POST["high_price"])
  ) {
    // Only query if we got a low and high price
    ...
    response.send(
      "Please go back and enter a price range (minimum and maximum). Thank you! :)"
    );
  }
}
...
function query_DB(POST, response) {
  if (
    isNonNegativeInteger(POST["low_stamp"]) &&
    ...
    query =
      "SELECT CustomerID, FName, LName, RewardStatus FROM Customer where RewardStatus > " +
      low +
      " and RewardStatus < " +
      high; // Build the query string
    con.query(query, function (err, result, fields) {
      // Run the query
      if (err) throw err;
      console.log(result);
